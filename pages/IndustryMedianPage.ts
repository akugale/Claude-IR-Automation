...
export class IndustryMedianPage extends BasePage {
  readonly table: TableComponent;
  readonly paginator: PaginatorComponent;
  readonly export: ExportComponent;

  private readonly financialYearInput: Locator;
  private readonly asOnDateInput: Locator;
  private readonly pbidtMarginInput: Locator;
  private readonly operatingCycleInput: Locator;
  private readonly currentRatioInput: Locator;
  ...
  constructor(private readonly page: Page) {
    ...
    this.financialYearInput = dialog.getByPlaceholder(/financial year/i).first();
    this.asOnDateInput = dialog.getByPlaceholder(/as on date/i).first();
    this.pbidtMarginInput = dialog.locator('input').nth(1);
    this.operatingCycleInput = dialog.locator('input').nth(2);
    this.currentRatioInput = dialog.locator('input').nth(3);
  ...
  async goto(): Promise<void> {
    ...
    await this.clickNavNode('Industry Parameter');
    await expect(this.page.getByRole('heading', { name: /industry parameter/i })).toBeVisible();
  }
  ...
  async verifyRequiredColumns(): Promise<void> {
    for (const col of ['Industry', 'PBIDT Margin', 'Operating Cycle', 'Current Ratio', 'As On Date']) {
      await expect(
  ...
  async verifyAddModalContents(): Promise<void> {
    ...
    await expect(this.financialYearInput).toBeVisible();
  ...
  async verifyAddButtonDisabledWhenIndustryNotSelected(): Promise<void> {
    await this.financialYearInput.fill('25');
    await expect(this.saveButton).toBeDisabled();
    await this.financialYearInput.clear();
  }

  async verifyAddButtonDisabledWhenFinancialYearEmpty(): Promise<void> {
    const firstOption = await this.getIndustryDropdownFirstOption();
    if (firstOption) await this.selectIndustry(firstOption);
    await this.financialYearInput.fill('');
    await expect(this.saveButton).toBeDisabled();
  }
  ...
  async getIndustryDropdownFirstOption(): Promise<string | null> {
    const opts = await this.getIndustryDropdownOptions();
    return opts.length > 0 ? opts[0] : null;
  }

  async selectIndustry(optionText: string): Promise<void> {
    const dropdown = this.page.locator('[role="dialog"] p-select').nth(0);
    await dropdown.click();
    const overlay = this.page.locator('.p-select-overlay').last();
    ...
    const option = overlay
      ...
      .filter({ hasText: new RegExp(`^\\s*${optionText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*$`, 'i') })
      .first();
  ...
  async fillAddFormFields(industry: string, financialYear: string, asOnDate?: string): Promise<void> {
    await this.selectIndustry(industry);
    await this.financialYearInput.fill(financialYear);
    if (asOnDate) {
      await this.asOnDateInput.fill(asOnDate);
    }
  }
  ...
  async addIndustryMedian(industry: string, financialYear: string, asOnDate?: string): Promise<void> {
    await this.openAddModal();
    await this.fillAddFormFields(industry, financialYear, asOnDate);
  ...
  async verifyViewModalIsReadOnly(): Promise<void> {
    ...
    for (const sel of selects) {
      ...
      expect(cls.includes('p-disabled'), 'View modal dropdown should be disabled').toBe(true);
    }
  }
  ...
  async editAndVerifyResetRestoresValues(rowText: string, tempValue: string): Promise<void> {
    await this.openEditModal(rowText);
    const original = await this.financialYearInput.inputValue();
    await this.financialYearInput.clear();
    await this.financialYearInput.fill(tempValue);
    await this.cancelButton.click();
    const modalHidden = !(await this.page.locator('[role="dialog"]').isVisible().catch(() => false));
    const afterCancel = await this.financialYearInput.inputValue().catch(() => '');
    expect(
  ...
  async verifyUpdateButtonDisabledWhenFinancialYearCleared(): Promise<void> {
    await this.financialYearInput.clear();
    await expect(this.updateButton).toBeDisabled();
  }

  async editAndUpdate(rowText: string, newFinancialYear: string): Promise<void> {
    await this.openEditModal(rowText);
    await this.financialYearInput.clear();
    await this.financialYearInput.fill(newFinancialYear);
  ...
  private async getAddButton(): Promise<Locator> {
    const headingBtn = this.page
      .getByRole('heading', { name: /industry parameter/i })
      .locator('..')
