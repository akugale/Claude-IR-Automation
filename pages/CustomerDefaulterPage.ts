...
export class CustomerDefaulterPage extends BasePage {
  ...
  async goto(): Promise<void> {
    ...
    await this.clickNavNode('Customer Defaulter');
    // Wait for table to confirm page loaded — heading check is done in TC_001
  ...
  async verifyScreenElements(): Promise<void> {
    await expect(this.page.getByRole('heading', { name: /(customer|counterparty) defaulter/i })).toBeVisible();
    await expect(this.page.locator('table')).toBeVisible();
