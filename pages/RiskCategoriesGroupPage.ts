...
export class RiskCategoriesGroupPage extends BasePage {
  ...
  constructor(private readonly page: Page) {
    ...
    this.codeInput = dialog.getByPlaceholder(/code/i).first();
    this.descriptionInput = dialog.getByPlaceholder(/description/i).first();
    this.saveButton = dialog.getByRole('button', { name: /^(add|save)$/i });
  ...
  async goto(): Promise<void> {
    ...
    await expect(this.page.getByRole('heading', { name: /risk categor.*group/i })).toBeVisible();
  }
  ...
  async verifyRequiredColumns(): Promise<void> {
    for (const col of ['Code', 'Description']) {
      await expect(
  ...
  async verifyNoExtraColumns(): Promise<void> {
    const headers = await this.page.locator('th').allInnerTexts();
    const cleaned = headers.map(h => h.replace(/\s+/g, ' ').trim()).filter(h => h.length > 0);
    const extra = cleaned.filter(
      h => !/^(code|description|status|created\s*by|created\s*date|actions?)$/i.test(h),
    );
    expect(extra, `Unexpected columns found: ${extra.join(', ')}`).toHaveLength(0);
  }
  ...
  async verifyViewModalIsReadOnly(): Promise<void> {
    ...
    for (const input of inputs) {
      ...
      expect(
        ...
        'View modal input should be disabled or read-only',
      ).toBe(true);
    }
  }
  ...
  async getAllDescriptions(): Promise<string[]> {
    ...
    const all: string[] = [];
    while (true) {
      const vals = await this.table.getAllColumnValues(1);
      all.push(...vals.filter(v => v.length > 0));
  ...
  private async getAddButton(): Promise<Locator> {
    const headingBtn = this.page
      .getByRole('heading', { name: /risk categor.*group/i })
      .locator('..')
