...
export class RatingAttributeMappingPage extends BasePage {
  readonly table: TableComponent;
  readonly paginator: PaginatorComponent;
  readonly export: ExportComponent;

  private readonly attributeIdInput: Locator;
  private readonly dataTypeInput: Locator;
  ...
  constructor(private readonly page: Page) {
    ...
    this.attributeIdInput = dialog.locator('input[type="number"], input[placeholder*="attribute" i], input[placeholder*="id" i]').first();
    this.dataTypeInput = dialog.getByPlaceholder(/enter data type/i).first();
  ...
  async goto(): Promise<void> {
    ...
    await this.clickNavNode('Rating Attribute Mapping');
    await expect(this.page.getByRole('heading', { name: /rating attribute mapping/i })).toBeVisible();
  }
  ...
  async verifyRequiredColumns(): Promise<void> {
    for (const col of ['Rating Parameter', 'Attribute ID', 'Data Type']) {
      await expect(
  ...
  async verifyAddModalContents(): Promise<void> {
    // Rating Parameter dropdown (nth 0)
    await expect(this.page.locator('[role="dialog"] p-select').nth(0)).toBeVisible();
    await expect(this.attributeIdInput).toBeVisible();
    await expect(this.dataTypeInput).toBeVisible();
  ...
  async verifyAddButtonDisabledWhenRatingParamEmpty(): Promise<void> {
    await this.dataTypeInput.fill('TestType');
    await this.attributeIdInput.fill('100');
    await expect(this.saveButton).toBeDisabled();
    await this.dataTypeInput.clear();
  }

  async verifyAddButtonDisabledWhenDataTypeEmpty(ratingParam: string): Promise<void> {
    await this.selectRatingParameter(ratingParam);
    await this.dataTypeInput.fill('');
    await expect(this.saveButton).toBeDisabled();
  }

  async getRatingParameterDropdownOptions(): Promise<string[]> {
  ...
  async selectRatingParameter(optionText: string): Promise<void> {
    const dropdown = this.page.locator('[role="dialog"] p-select').nth(0);
    await dropdown.click();
    const overlay = this.page.locator('.p-select-overlay').last();
  ...
  async fillAddFormFields(ratingParam: string, attributeId: string, dataType: string): Promise<void> {
    await this.selectRatingParameter(ratingParam);
    await this.attributeIdInput.fill(attributeId);
    await this.dataTypeInput.fill(dataType);
  }
  ...
  async addAttributeMapping(ratingParam: string, attributeId: string, dataType: string): Promise<void> {
    await this.openAddModal();
    await this.fillAddFormFields(ratingParam, attributeId, dataType);
  ...
  async editAndVerifyResetRestoresValues(rowText: string, tempDataType: string): Promise<void> {
    await this.openEditModal(rowText);
    const original = await this.dataTypeInput.inputValue();
    await this.dataTypeInput.clear();
    await this.dataTypeInput.fill(tempDataType);
    await this.cancelButton.click();
    const afterCancel = await this.dataTypeInput.inputValue().catch(() => '');
    // Cancel closes modal OR restores — either is acceptable
  ...
  async verifyUpdateButtonDisabledWhenDataTypeCleared(): Promise<void> {
    await this.dataTypeInput.clear();
    await expect(this.updateButton).toBeDisabled();
  }

  async editAndUpdate(rowText: string, newDataType: string): Promise<void> {
    await this.openEditModal(rowText);
    await this.dataTypeInput.clear();
    await this.dataTypeInput.fill(newDataType);
  ...
  private async getAddButton(): Promise<Locator> {
    const headingBtn = this.page
      .getByRole('heading', { name: /rating attribute mapping/i })
      .locator('..')
